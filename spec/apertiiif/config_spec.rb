...
RSpec.describe Apertiiif::Config do
  before(:context) do
    @seed   = { 'label' => 'my batch label' }
    @config = Apertiiif::Config.new(@seed)
  end
  describe '.new' do
    context 'when given the minimum metadata (label)' do
      it 'creates a config object' do
        expect(@config).to be_an(Apertiiif::Config)
      end
    end
  ...
  describe '.batch_namespace' do
    it 'deduces namespace from current directory' do
      @dir    = '/Users/fake/supernot_realATall/apertiiif-batch_result'
