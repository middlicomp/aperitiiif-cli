...
RSpec.describe Apertiiif::Records do
  before(:all) do
  ...
  describe '.records' do
    it 'returns an array of records' do
      expect(@batch.records.first).to be_an(Apertiiif::Record)
    end
  end
  ...
  describe '.find_record' do
    context 'when given a record id that exists' do
      it 'returns the record' do
        @id = 'valid'
        expect(@batch.find_record(@id)).to be_an(Apertiiif::Record)
      end
    end
  ...
  describe '.records_from_file' do
    ...
    context 'when the configured records file exists' do
      it 'returns an array of records' do
        expect(@batch.records_from_file.first).to be_an(Apertiiif::Record)
      end
    end
  end
